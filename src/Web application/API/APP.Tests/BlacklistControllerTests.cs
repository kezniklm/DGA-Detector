...
public class BlacklistControllerTests : IClassFixture<ApiApplicationFactory<Program>>, IAsyncLifetime
{
    ...
    {
        // Arrange
        BlacklistModel invalidBlacklist = new() { DomainName = "null", Added = default, Id = default };
        const string url = "/Blacklist/";

        // Act
        HttpResponseMessage response = await _client.PostAsJsonAsync(url, invalidBlacklist);
